...
const EditConsolidation = ({
  ...
  const [address, setAddress] = useState({
    isFocused: false,
    selectAddress: consolidationItem.address,
    searchAddress: consolidationItem.address,
    error: "",
  });
  ...
  const [selectPickupPointList, setSelectPickupPointList] = useState(
    consolidationItem.pickupPointList.map((item) => {
      return { openDropdown: false, selectedPickupPoint: item, error: "" };
    })
  );
  ...
  const handleEdit = async () => {
    ...
    fetch(
      `${API.baseURL}/api/productConsolidationArea/updateInfo?productConsolidationAreaId=${consolidationItem.id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
      ...
      .then((respond) => {
        ...
        fetch(
          `${API.baseURL}/api/productConsolidationArea/updatePickupPointList`,
          {
            ...
            body: JSON.stringify({
              id: consolidationItem.id,
              newUpdateIdList: selectPickupPointList.map(
                (item) => item.selectedPickupPoint.id
              ),
            }),
          }
          ...
          .then((res) => {
            fetch(
              `${
                API.baseURL
              }/api/productConsolidationArea/getAllForAdmin?page=${
                page - 1
  ...
  return (
    ...
    >
      ...
      <div className="modal__container-header">
        <h3 className="modal__container-header-title">Sửa điểm tập kết</h3>
